	...
	public OptionsPanel(Frame parent)
	{
		...
		forceButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent ae)
			{
				...
				forceButton.setText("Sera faite!");
		...
		/* labelPanel.add(new JLabel("Localisation sur le disque: ", 4)); */
		/*
		 ...
		 * int x = 0; int y = 0;
		 * 
		 * FontMetrics fm = g.getFontMetrics(); int width = fm.stringWidth(getText()); int height = fm.getHeight();
		 * 
		 * if (getAlignmentX() == 2.0F) x = 0; else if (getAlignmentX() == 0.0F) x = getBounds().width / 2 - width / 2; else if (getAlignmentX() == 4.0F) x =
		 * getBounds().width - width; y = getBounds().height / 2 + height / 2 - 1;
		 * 
